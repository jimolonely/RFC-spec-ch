...
响应的第一行，包含协议版本，状态码，和文本说明。

```shell script
Status-Line = HTTP-Version SP Status-Code SP Reason-Phrase CRLF
```

### 6.1.1 Status Code 和 文本说明

状态码是3位数字，文本说明是对状态码的简短描述。

```shell script
- 1xx: Informational - 收到请求，继续处理

- 2xx: Success - 成功 received, understood, and accepted
...
* 打开和关闭更少的TCP连接，可以节省CPU时间，节省内存
* 连接里的请求响应可以管道话，不用等每个请求响应就可以发送下一个请求
* 通过减少包的数量减少网络阻塞
* 减少TCP的握手来减少延迟时间
...
同样的请求请求多次应该是一个效果。但一系列请求不一定是冥等的。
